...
class Call:
    ...
    async def push_turn(self, uid: str, event: Any) -> dict | None:
        ...
        async with self.lock:
            ...
            if p is None:
                raise CallError(403, "not a participant of this call")
            # CONSENT FIRST, then de-duplication. A pending observer is not in
            # the conversation: her words never enter the merged transcript (and
            # so never enter a participant's episode of a call she was not let
            # into). That has to be decided before anything else looks at the
            # payload — checking dedup first could return an already-merged turn
            # to someone who is not yet allowed to be here.
