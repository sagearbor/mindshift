...
class RoutingLLM:
    ...
    Every nudge is a DIFFERENT line. ``audio_pipeline._gate_nudge`` silences a
    nudge that repeats a recent one (word-bigram Jaccard >=
    COACH_REPEAT_JACCARD within COACH_REPEAT_COOLDOWN_S session-seconds), so a
    double that always answered "ease up" would have the coach correctly fall
    silent from the SECOND self turn on — and a call test waiting for that
    turn's coaching would wait forever for a frame the server is right not to
    send. A real model does not repeat itself verbatim either. The nudge TEXT
    is never what these tests assert on; that the turn reached the model at
    all (``prompts``) is."""

    #: Distinct nudges, handed out in order — no two consecutive ones share a
    #: word bigram, so none is ever gated as a repeat.
    NUDGES = ("ease up", "slow down", "breathe first",
              "soften that", "hold steady", "be firmer")
