...
for k in MINDSHIFT_MODEL STT_PROVIDER WHISPER_MODEL MINDSHIFT_ALLOWED_ORIGINS MINDSHIFT_RECORDINGS_BUCKET LOG_LEVEL RATE_LIMIT_ENABLED RATE_LIMIT_PER_MINUTE MINDSHIFT_DIARIZE_CROSSCHECK MINDSHIFT_DIARIZE_MAX_POOLED_COSINE MINDSHIFT_DIARIZE_SPLIT_MIN_MARGIN; do
  v="${!k:-}"
  [[ -n "$v" ]] || v="$(read_env "$k")"
  if [[ -n "$v" ]]; then
