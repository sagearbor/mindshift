...
export function useAudioStream(
  ...
  const handleAudioBuffer = useCallback(
    (buffer: AudioStreamBuffer) => {
      ...
      // server needs to re-anchor its timeline on reconnect. Journal frames
      // returned above are deliberately NOT counted: they never enter
      // `pendingRef`, so counting them would push `last_local_time` past the
      // audio the server could ever have received.
